...
class VendorDim(BaseFactETL):
    ...
    def run(self):
        ...
        # Load Source Data
        sql_get_vendors = """
                            SELECT LIFNR, NAME, LAND1, REGIO,VENDOR_TYPE
                            FROM SAPSR3.ZCON_V_VENDOR                                                       
                            WHERE CHDAT = :yesterday
                        """

        results: pd.DataFrame = pd.read_sql(
            sql_get_vendors, con=self._con_sap, params={"yesterday": yesterday}
        )

        if results.empty:
            Logger().info("No vendors found for processing")
            with self._con_dw.begin() as conn:
                self._update_etl_info(conn, "process_vendors")
            return
        ...
        results["region_search_key"] = (
            results["CountryId"].fillna(0).astype("Int64").astype(str)
            + results["regio"]
